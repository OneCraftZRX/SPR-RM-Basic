...
class ncsyprocs(object):
    ...
    def thresholdplus(src,show,val=130):
        gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
        ret, dst = cv2.threshold(gray, val, 255, cv2.THRESH_BINARY)
    ...
    def open_with_threshold(image,show,val,knl):
        binary=ncsyprocs.thresholdplus(image,0,val)
    ...
    def close_with_threshold(image,show,val,knl):
        binary=ncsyprocs.thresholdplus(image,0,val)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (knl, knl))
